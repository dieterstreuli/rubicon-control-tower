...
async function main() {
  ...
  // Stufe 5: Kalender/Eskalation — Rollen-Gate + id-Guard (option-aussehende id → 400, kein Flag-Aliasing)
  await expect('kalender/event: lesende Rolle → 403', '/api/kalender/event', { body: { ...LESEND, id: 'IN-02' } }, 403)
  await expect('kalender/event: id fehlt → 400', '/api/kalender/event', { body: { ...CoS } }, 400)
  await expect('kalender/event: option-aussehende id → 400', '/api/kalender/event', { body: { ...CoS, id: '--subject' } }, 400)
  await expect('eskalation/mail: lesende Rolle → 403', '/api/eskalation/mail', { body: { ...LESEND, id: 'IN-02' } }, 403)
  await expect('eskalation/mail: id fehlt → 400', '/api/eskalation/mail', { body: { ...CoS } }, 400)
  await expect('eskalation/mail: option-aussehende id → 400', '/api/eskalation/mail', { body: { ...CoS, id: '-x' } }, 400)
