...
| `scripts/gen_report.py` | Verdichtete Reports Woche/Monat/Quartal → PDF (`public/reports/`) + Google Doc (Drive: RUBICON — Reports); Eskalations-Filter GL/VR. **01.08.: Wochen-Report enthält Δ-Block (gen_delta); `--ki` = optionaler KI-ENTWURF** (Narrativ zum Berichtszeitraum + 2-Satz-Ampel-Begründung je rot/gelb, alle Ebenen + Auto-Job; über die Modell-Fassade `_tools/ai_client.py` — lokal CLI / Server Vertex; fakten-gebunden, klar markiert, Fehler/Leerfall sichtbar im Report). Checkbox im Reports-Tab. |
